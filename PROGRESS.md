...
Updated: 2026-09-23T23:55:00+08:00

The connection monitor and CLI viewer are implemented. The current offline suites pass 25/25 monitor checks and 18/18 retry checks; the Windows PowerShell 5.1 viewer regression passes 1/1. The three installed files match the development copies by SHA-256. A fresh `opencode models` process loaded the plugin and wrote a `connection-configured` status row. Already-running desktop processes retain their old in-memory plugin code until restarted.
